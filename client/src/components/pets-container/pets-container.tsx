	...
	return (
		<div>
          ...
          {currentUser.pets.map((pet: any, index: number) => (
						<Pet key={index} {...pet} isFollowing isLoggedIn imageUrl='../../images/sample.jpeg' />
					))}
          </div>
				</div>
			) : null}
			...
			<div className='all-pets'>
				...
				<div className='pets-container'>
					{pets.pets.map((pet: any, index: number) => (
						<Pet key={index} {...pet} isFollowing={false} isLoggedIn={!!currentUser} imageUrl='../../images/sample.jpeg' />
					))}
				</div>
			</div>
		</div>
